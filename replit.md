...
- Phone Auth (Native): Uses Firebase Admin SDK (`firebase-admin`) on the backend. No reCAPTCHA or WebView needed. Flow: app sends phone number to `/api/auth/phone/send-code` → backend generates 6-digit OTP (stored in-memory with 5-min expiry) → app enters code → sends to `/api/auth/phone/verify-code` → backend verifies OTP, creates/gets Firebase user via `adminAuth.getUserByPhoneNumber()`/`adminAuth.createUser()`, generates custom token via `adminAuth.createCustomToken()` → client signs in with `signInWithCustomToken()`. Rate-limited (3 req/min per IP/phone). SMS delivery: currently logs OTP to server console (dev mode); for production, integrate Twilio or similar SMS provider.
