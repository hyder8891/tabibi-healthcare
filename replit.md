...
- Phone Auth (Native): Uses Firebase Admin SDK + Twilio SMS on the backend. No reCAPTCHA or WebView needed. Flow: app sends phone number to `/api/auth/phone/send-code` → backend generates 6-digit OTP (stored in-memory with 5-min expiry) → sends real SMS via Twilio (`server/twilio.ts`) → app enters code → sends to `/api/auth/phone/verify-code` → backend verifies OTP, creates/gets Firebase user via `adminAuth.getUserByPhoneNumber()`/`adminAuth.createUser()`, generates custom token via `adminAuth.createCustomToken()` → client signs in with `signInWithCustomToken()`. Rate-limited (3 req/min per IP/phone). Dev fallback: if Twilio fails in development mode, OTP is logged to server console. Twilio connected via Replit integration (connector). Requires Iraq (+964) enabled in Twilio Geo Permissions for Iraqi phone numbers.
